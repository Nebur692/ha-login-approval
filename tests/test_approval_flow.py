...
async def test_approved():
    with patch("app.approval_flow.ha_client.get_ha_language", AsyncMock(return_value="en")), \
        ...
        outcome = await run_approval(["mobile_app_x"], "req-1", CONTEXT)
...
async def test_all_sends_failing_short_circuits_without_waiting():
    ...
    with patch("app.approval_flow.ha_client.get_ha_language", AsyncMock(return_value="en")), \
        ...
        outcome = await run_approval(["mobile_app_x", "mobile_app_y"], "req-1", CONTEXT)
